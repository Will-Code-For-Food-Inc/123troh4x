...
struct RunOpParams {
    /// Session ID returned by start_session
    session_id: String,
    /// Operation to run: build, clean, check, disassemble, hex_dump, grep, git_status, git_diff, list_ops
    op: String,
...
impl RomhackServer {
        ...
        match podman::get_platform(&platform) {
            ...
            Some(info) => {
                ...
                let dockerfile = std::fs::read_to_string(
                    repo_root.join("platforms").join(info.name).join("Dockerfile"),
                )
                .unwrap_or_else(|e| format!("(could not read Dockerfile: {e})"));
                let launch = std::fs::read_to_string(
                    repo_root.join("platforms").join(info.name).join(info.image),
                )
    ...
    /// Start a persistent container session for a platform. Returns a session ID.
    #[tool(
    ...
    fn run_op(&self, Parameters(RunOpParams { session_id, op, params, workdir }): Parameters<RunOpParams>) -> String {
        ...
        // Reconstruct the typed op from the string name + params object.
        let op_value = {
...
#[tool_handler]
impl ServerHandler for RomhackServer {
